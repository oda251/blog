...
export async function fetchOlderTweets(
  tweetId: string,
  tagFilter: string | null
): Promise<Result<TweetWithTags[]>> {
  const url =
    `/api/tweets` +
    `?oldId=${tweetId}` +
...
export async function fetchNewerTweets(
  tweetId: string,
  tagFilter: string | null
): Promise<Result<TweetWithTags[]>> {
  const url =
...
export async function fetchTweets(
  ...
  const url = `/api/tweets` + (tagFilter ? `?tagId=${tagFilter}` : '');
  let result: Result<TweetWithTags[]> = { status: 200, data: [] };
  try {
    const response = await axios.get(url);
    result.data = response.data as TweetWithTags[];
