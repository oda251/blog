...
const TweetView = forwardRef<HTMLDivElement, TweetProps>((props, ref) => {
  ...
  return (
    <div ref={ref} className={`p-4 shadow-md rounded-lg relative ${props.className}`}>
      <Linkify
        as='p'
      ...
      <p className="tweet-author text-xs text-gray-400 mb-1 break-words">By {props.tweet.author === "" ? "名無しさん" : props.tweet.author}</p>
      ...
      <DeleteTweetButton tweetId={props.tweet.id!} className="absolute bottom-0 right-0 text-slate-500 px-3 py-3" />
    </div>
  );
});
